...
<table>
    <tr>
        <td class="no-borded" style="font-size: 14">Número do contrato: <span><?=$contrato['numero']?></span></td>
        <td class="no-borded" style="font-size: 14">Data de Contratação: <span><?php echo formata_data_br($contrato['dt_contratacao']);?></span></td>
        <td class="no-borded" style="font-size: 14">Data de Vencimento: <span><?php echo formata_data_br($contrato['dt_vencimento']);?></span></td>
    </tr>
</table>
<br>
...
<p>1.   O TITULAR CONTRATANTE pagará uma taxa de adesão do plano <b><?=$plano['nome']?></b>, com validade de <b><?=convertCodigoValidadePlanosAnos($plano['validade'])?></b>, a quantia total de <b>R$ <?= formata_preco($contrato['pag_total'])?></b> para tornar-se apto a desfrutar dos serviços ofertados pela rede credenciada CIMA SAÚDE com a opção de pagamento <b><?=$contrato['modo']?></b>,

<?php if($contrato['pag_texto'] != ""){?>

 pagos em: <?php echo '<b>' . $contrato['pag_texto']. '</b>, a saber:</p>'?>
...
<?php } else {?>
 com vencimento em: <b><?=formata_data_br($contas[0]['dt_recebimento'])?></b>. </p>
...
<p>6.   Os boletos com atraso maior que 30 dias serão levados a protesto.</p>
...
<h5 style="color:black"><?=dataContrato('Ourinhos/SP', $contrato['dt_contratacao'])?></h5>
