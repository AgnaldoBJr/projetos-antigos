<?php
	...
	class Proposta extends CI_Controller {
		...
		//*******************************************
		//****** MÉTODOS PARA SALVAR AS PROPOSTAS ***
		//*******************************************
		...
		public function novo(){
			...
			$table = "pessoas";
			$camposDeProjecao = "cod_pessoa, nome";
		...
		public function pagamento(){
			...
			if($this->form_validation->run()){
				...
				if(isset($data['indicacao']) && $data['indicacao'] == 1){
					$dataPagamento['indicacao'] = $data['indicacao'];
					$dataPagamento['indicacao_nome'] = $data['indicacao-nome'];
					$dataPagamento['indicacao_celular'] = $data['indicacao-celular'];
				} else{
					$dataPagamento['indicacao'] = 0;
				}
				...
				if(isset($data['cortesia']) && $data['cortesia']==1){
					//var_dump("Desculpe! estamos trabalhando nisso..."); die;
					$this->load->view("contratos/propostas/propostas-cortesia-teste", $dataPagamento);
					return;
				}
		...
		public function verificar(){
			...
			if($this->input->post('indicacao')!=null && $this->input->post('indicacao') == 1){
					$data['indicacao'] = $this->input->post('indicacao');
					$data['indicacao_nome'] = $this->input->post('indicacao-nome');
					$data['indicacao_celular'] = $this->input->post('indicacao-celular');
			} else{
				$data['indicacao'] = 0;
			}
			...
			//var_dump($this->input->post('pag-modo'), $data['pagamento']['modo']); die;
		...
		public function finalizar(){
			...
			if($this->input->post('proposta-status') == "Cancelar"){
				...
				if($this->input->post('dep_nome') != null)
					$dependentes = tratarDadosPessoas($this->input->post('dep_nome'), $this->input->post('dep_data'), $this->input->post('dep_parentesco'), count($this->input->post('dep_nome')), 2);
				if($this->input->post('agr_nome') != null)
					$agregados = tratarDadosPessoas($this->input->post('agr_nome'), $this->input->post('agr_data'), $this->input->post('agr_parentesco'), count($this->input->post('agr_nome')), 3);
				if($this->input->post('colab_nome') != null)
					$colaboradores = tratarDadosColaboradores($this->input->post('colab_nome'), $this->input->post('colab_data'), count($this->input->post('colab_nome')), 4);
				...
				//SALVANDO DADOS DE INDICAÇÕES
				if($this->input->post('indicacao') != 0){
				//Preparando dados
					$data['indicacao'] = parseIndicacao($this->input->post());
				//Inserindo no banco de dados
					$r = $this->Generic_model->insert('indicacoes', 'cod_indicacao', $data['indicacao']);
				}
		...
		//***********************************************************
		//******     MÉTODOS DE VISUALIZAÇÃO DAS PROPOSTAS  *********
		//***********************************************************
		...
		public function ganhar(){
			...
			//Gerando e salvando os números de carteirinhas
			$this->gerarCarteirinhas($data['cod_proposta']);
		...
		public function gerarCarteirinhas($proposta){
			//Buscar dados
			$data['proposta'] = $this->Generic_model->readById('propostas', 'cod_proposta', $proposta);

			$titular = $this->Generic_model->readAllWhere('clientes', 'cod_cliente', $data['proposta']['fk_cliente']);

			$dependentes= $this->Generic_model->readAllWhere('propostas_dependentes', 'fk_proposta', $proposta);

			$agregados = $this->Generic_model->readAllWhere('propostas_agregados', 'fk_proposta', $proposta);

			$colaboradores = $this->Generic_model->readAllWhere('propostas_colaboradores', 'fk_proposta', $proposta);

			//var_dump($titular, $dependentes, $agregados, $colaboradores); 

			//Gerrar números
			$i = 1;
			if($titular){
				for($k = 0; $k < count($titular); $k++){
					$titular[$k]['carteirinha'] = $this->gerarNumeroCarteirinha($titular[0]['cod_cliente'], $i);
					$i += 1;
				}
			}

			if($dependentes){
				for($k = 0; $k < count($dependentes); $k++){
					$dependentes[$k]['carteirinha'] = $this->gerarNumeroCarteirinha($titular[0]['cod_cliente'], $i);
					$i += 1;
				}
			}

			if($agregados){
				for($k = 0; $k < count($agregados); $k++){
					$agregados[$k]['carteirinha'] = $this->gerarNumeroCarteirinha($titular[0]['cod_cliente'], $i);
					$i += 1;
				}
			}

			if($colaboradores){
				for($k = 0; $k < count($colaboradores); $k++){
					$colaboradores[$k]['carteirinha'] = $this->gerarNumeroCarteirinha($titular[0]['cod_cliente'], $i);
					$i += 1;
				}
			}
			//var_dump('--------------------', $titular, $dependentes, $agregados, $colaboradores, $i, count($titular)); die;

			//Salvar tudo... :)
			if($titular){
				for($k = 0; $k < count($titular); $k++){
					$data = array('carteirinha' => $titular[$k]['carteirinha'], 'dt_carteirinha' => date("Y-m-d"));
					$this->Generic_model->update('clientes', 'cod_cliente', $titular[$k]['cod_cliente'], $data);
				}
			}

			if($dependentes){
				for($k = 0; $k < count($dependentes); $k++){
					$data = array('carteirinha' => $dependentes[$k]['carteirinha'], 'dt_carteirinha' => date("Y-m-d"));
					$this->Generic_model->update('propostas_dependentes', 'cod_dependente', $dependentes[$k]['cod_dependente'], $data);
				}
			}

			if($agregados){
				for($k = 0; $k < count($agregados); $k++){
					$data = array('carteirinha' => $agregados[$k]['carteirinha'], 'dt_carteirinha' => date("Y-m-d"));
					$this->Generic_model->update('propostas_agregados', 'cod_agregado', $agregados[$k]['cod_agregado'], $data);
				}
			}

			if($colaboradores){
				for($k = 0; $k < count($colaboradores); $k++){
					$data = array('carteirinha' => $colaboradores[$k]['carteirinha'], 'dt_carteirinha' => date("Y-m-d"));
					$this->Generic_model->update('propostas_colaboradores', 'cod_colaborador', $colaboradores[$k]['cod_colaborador'], $data);
				}
			}

		}

		public function gerarNumeroCarteirinha($cod, $i){
			$clinica = "001";
			$ano = date('y');
			...
			return $clinica . ' ' . $ano . $cod_titular . ' ' . $numero;
		}
		...
		public function novoPDF($cod){
			...
			//var_dump($data['dependentes']); die;
				...
				$html = $this->load->view('contratos/contrato-template/template-proposta', $data, TRUE);
				$mpdf->AddPage('','','','','');
		...
		//METODO QUE NÃO É UTILIZADO (REFATORAÇÃO)
		//Apenas de exemplo
		...
		public function pdf_relatorio($data){
			$mpdf = new mPDF();
			$html = $this->load->view('relatorios/templates/propostas', $data, TRUE);
			$mpdf->AddPage('','','','','');
			$mpdf->SetHeader('CIMA SAÚDE');
			$mpdf->SetFooter('
				<table width="100%" style="vertical-align: bottom; font-family: serif; font-size: 8pt; color: #000000; font-weight: bold; font-style: italic; border: none; border-collapse:collapse;"> 
			...
			');
			$mpdf->writeHTML($html);
			$mpdf->Output('Relatorio-Propostas'. date("Ymd").'.pdf' , 'D');
			 redirect('propostas');
		}
